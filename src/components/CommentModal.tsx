...
const CommentModal: React.FC<CommentModalProps> = ({ post, isOpen, onClose }) => {
  ...
  const { getCommentsByPostId, addComment, likeComment, unlikeComment } = useComments();
  ...
  const handleCommentLike = (commentId: string, isLiked: boolean) => {
    ...
    isLiked ? unlikeComment(commentId, currentUser.id) : likeComment(commentId, currentUser.id);
  };
