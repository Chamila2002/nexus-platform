...
import { TrendingUp, Users as UsersIcon, Hash, Sparkles } from 'lucide-react';
...
const Explore: React.FC = () => {
  ...
  // load posts + users once
  useEffect(() => {
    (async () => {
      const [p, u] = await Promise.all([PostsApi.list(1, 100), UsersApi.list()]);
      setPosts(p.items);
      setAllUsers(u);
    })();
  }, []);
  ...
  const trendingPosts = useMemo(() => {
    return [...posts]
      .sort((a, b) => {
        const scoreA = (a.likes ?? 0) * 2 + (a.comments ?? 0) * 3 + (a.shares ?? 0) * 4;
        const scoreB = (b.likes ?? 0) * 2 + (b.comments ?? 0) * 3 + (b.shares ?? 0) * 4;
  ...
  const mediaPosts = useMemo(() => posts.filter(p => !!p.imageUrl), [posts]);

  const suggestedUsers = useMemo(() => {
    return [...allUsers].sort((a, b) => (b.followers ?? 0) - (a.followers ?? 0)).slice(0, 5);
  }, [allUsers]);
  ...
  const getPostsForTab = () => {
    if (isSearching) {
      return searchResults.filter(r => r.type === 'post').map(r => r.data as Post);
    }
    switch (activeTab) {
      case 'trending':
  ...
  const tabs = [
    ...
    { key: 'people', label: 'People', icon: UsersIcon, count: suggestedUsers.length },
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 text-gray-900 dark:text-gray-100">
      <div className="mb-6">
        ...
        <p className="text-gray-600 dark:text-gray-400">Discover trending posts and connect with new people</p>
      </div>

      <div className="mb-6">
        <SearchBar onSearchResults={handleSearchResults} placeholder="Search posts, people, and topics..." />
      </div>
          ...
          {isSearching ? (
            <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {searchResults
                      ...
                      .map((r, i) => {
                        ...
                        return (
                          <div key={i} className="flex items-center space-x-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            <img
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-2">
                                <p className="font-semibold truncate text-gray-900 dark:text-white">{user.displayName}</p>
                                {user.verified && (
                              ...
                              <p className="text-sm text-gray-600 dark:text-gray-300">{(user.followers ?? 0).toLocaleString()} followers</p>
                            </div>
                          </div>
                        );
                      })}
              ...
              {searchResults.some(r => r.type === 'post') && (
                <div>
                  <h3 className="text-md font-medium text-gray-700 dark:text-gray-300 mb-3">Posts</h3>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {suggestedUsers.map((user) => (
                  <div key={user.id} className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6">
                    <div className="flex items-center space-x-4 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center space-x-2">
                          <h3 className="font-semibold truncate text-gray-900 dark:text-white">{user.displayName}</h3>
                          {user.verified && (
                    ...
                    {user.bio && <p className="text-gray-700 dark:text-gray-300 text-sm mb-4 line-clamp-2">{user.bio}</p>}
            ...
            <div>
              <h2 className="text-lg font-semibold mb-4">{activeTab === 'trending' ? 'Trending Posts' : 'Latest Posts'}</h2>
              <PostFeed posts={getPostsForTab()} />
            </div>
          )}
        </div>
